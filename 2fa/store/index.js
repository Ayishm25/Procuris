...
export const sendVerification = createAsyncThunk('authentication/sendverification', async (data, thunkAPI) => {
  try {
    const response = await api.sendVerification(data, thunkAPI?.getState().auth.token);
    return response.data;
  } catch (error) {
    ShowToast(mapErrors(error));
    throw new Error(mapErrors(error));
  }
});
export const getGoogleAuthenticatorQR = createAsyncThunk('authentication/getGoogleAuthenticatorQR', async (data, thunkAPI) => {
  try {
    const response = await api.getGoogleAuthenticatorQR(thunkAPI?.getState().auth?.token);
    return response.data;
  } catch (error) {
    ShowToast(mapErrors(error));
    throw new Error(mapErrors(error));
...
export const verifyCode = createAsyncThunk('authentication/verifyCode', async (data, thunkAPI) => {
  try {
    const response = await api.verifyCode(data, thunkAPI?.getState().auth?.token);
    return response.data;
  } catch (error) {
...
export const verifyEnableAuthenticationCode = createAsyncThunk('authentication/verifyEnableAuthenticationCode', async (data, thunkAPI) => {
  try {
...
export const checkAuthenticationStatus = createAsyncThunk('authentication/checkAuthenticationStatus', async (payload, thunkAPI) => {
  try {
    const response = await api.checkAuthenticationStatus(payload ? payload : util.Interceptor.getToken());
    return response;
  } catch (error) {
    throw new Error(mapErrors(error));
  }
});
export const disableAuthenticationStatus = createAsyncThunk('authentication/disableAuthenticationStatus', async (payload, thunkAPI) => {
  try {
    const response = await api.disableAuthenticationStatus(thunkAPI.getState()?.auth?.token);
    return response.data;
  } catch (error) {
    Alert.alert('Error', mapErrors(error));
    throw new Error();
